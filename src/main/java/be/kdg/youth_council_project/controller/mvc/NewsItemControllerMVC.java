...
public class NewsItemControllerMVC {
    ...
    @GetMapping("news-item/{newsItemId}")
    public ModelAndView getNewsItem(@PathVariable long newsItemId) {
        LOGGER.info("Getting one news item for youth council");
        ModelAndView modelAndView = new ModelAndView("modules/news-item");
        NewsItem newsItems = newsItemService.getNewsItemById(newsItemId);
        modelAndView.addObject("newsItem", newsItems);
        return modelAndView;
    }
}
