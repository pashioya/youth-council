...
const MultiselectDropdown = () => {
  const newEl = (tag, attrs) => {
    ...
    if (attrs !== undefined) {
      Object.keys(attrs).forEach((k) => {
        if (k === 'class') {
          Array.isArray(attrs[k]) ? attrs[k].forEach((o) => o !== '' ? el.classList.add(o) : 0) : (attrs[k] !== '' ? el.classList.add(attrs[k]) : 0);
        } else if (k === 'style') {
          Object.keys(attrs[k]).forEach((ks) => {
            el.style[ks] = attrs[k][ks];
          });
        } else if (k === 'text') {
          attrs[k] === '' ? el.innerHTML = '&nbsp;' : el.innerText = attrs[k];
        } else {
          el[k] = attrs[k];
        }
      });
  ...
  document.querySelectorAll('select[multiple]').forEach((el, k) => {
    const div = newEl('div', {class: 'multiselect-dropdown', style: {width: el.clientWidth + 'px', padding: ''}});
    el.style.display = 'none';
    el.parentNode.insertBefore(div, el.nextSibling);
    const listWrap = newEl('div', {class: 'multiselect-dropdown-list-wrapper'});
    const list = newEl('div', {class: 'multiselect-dropdown-list', style: {height: '15rem'}});
    const search = newEl('input', {
      class: ['multiselect-dropdown-search'].concat(['form-control']),
    ...
    el.loadOptions = () => {
      ...
      if (el.attributes['multiselect-select-all']?.value === 'true') {
        ...
        op.appendChild(ic);
        op.appendChild(newEl('label', {text: 'All'}));
      ...
      div.refresh = () => {
        ...
        if (sels.length > (el.attributes['multiselect-max-items']?.value ?? 5)) {
          div.appendChild(newEl('span', {class: ['optext', 'maxselected'], text: sels.length + ' selected'}));
        } else {
          sels.map((x) => {
        ...
        if (el.selectedOptions.length === 0) {
          div.appendChild(newEl('span', {class: 'placeholder', text: el.attributes['placeholder']?.value ?? 'select'}));
        }
      };
      div.refresh();
    };
