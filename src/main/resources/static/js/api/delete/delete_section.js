...
function deleteClicked(event) {
    ...
    const divId = div.id
    const sectionId = +divId.substring(divId.indexOf('_') + 1);
...
function handleDeletionResponse(response) {
    if (response.status === 204) {
        const sectionId = +response.url.substring(response.url.lastIndexOf('/') + 1);
        const div = document.querySelector(`#section_${sectionId}`);
    }
}
